...
public class UniGifImage : MonoBehaviour
{
    ...
    public void setImgAspectCtrl(UniGifImageAspectController uniGifImageAspectController)
    {
        this.m_imgAspectCtrl = uniGifImageAspectController;
    }
    ...
    public PlayableGif playableGif { get;  set; }
    ...
    private void Update()
    {
        switch (nowState)
        {
            ...
            case State.Playing:
                if (m_rawImage == null || playableGif==null
                    ||playableGif.gifTextureList == null || playableGif.gifTextureList.Count <= 0)
                {
                    return;
                }
                ...
                // Change texture
                m_gifTextureIndex++;
                if (m_gifTextureIndex >= playableGif.gifTextureList.Count)
                {
                    m_gifTextureIndex = 0;

                    if (playableGif.loopCount > 0)
                    {
                        m_nowLoopCount++;
                        if (m_nowLoopCount >= playableGif.loopCount)
                        {
                            Stop();
                            return;
                        }
                ...
                m_rawImage.texture = playableGif.gifTextureList[m_gifTextureIndex].m_texture2d;
                m_delayTime = Time.time + playableGif.gifTextureList[m_gifTextureIndex].m_delaySec;
                break;
    ...
    {
        StartCoroutine(SetGifFromUrlCoroutine(url,url, autoPlay));
    }
    ...
    public IEnumerator SetGifFromUrlCoroutine(string matName,string url, bool autoPlay = true)
    {
        ...
        yield return StartCoroutine(UniGif.GetTextureListCoroutine(request.downloadHandler.data, (gifTexList, loopCount, width, height) =>
        {
            ...
            if (gifTexList != null)
            {
                ...
                this.playableGif = new PlayableGif(matName, loopCount, width, height, gifTexList);
                print("gif解码完毕");
                DataMap.matGifCollection.AddPlayable(matName, playableGif);
    ...
    public void Clear()
    {
        ...
        if (playableGif!=null && playableGif.gifTextureList != null)
        {
            for (int i = 0; i < playableGif.gifTextureList.Count; i++)
            {
                if (playableGif.gifTextureList[i] != null)
                {
                    if (playableGif.gifTextureList[i].m_texture2d != null)
                    {
                        Destroy(playableGif.gifTextureList[i].m_texture2d);
                        playableGif.gifTextureList[i].m_texture2d = null;
                    }
                    playableGif.gifTextureList[i] = null;
                }
            }
            playableGif.gifTextureList.Clear();
            playableGif = null;
        }
    ...
    public void Play()
    {
        ...
        if (m_rawImage == null || playableGif==null || playableGif.gifTextureList == null || playableGif.gifTextureList.Count <= 0)
        {
            Debug.LogError("Raw Image or GIF Texture is nothing.");
            return;
        }
        nowState = State.Playing;
        m_rawImage.texture = playableGif.gifTextureList[0].m_texture2d;
        m_delayTime = Time.time + playableGif.gifTextureList[0].m_delaySec;
        m_gifTextureIndex = 0;
        m_nowLoopCount = 0;
    }
